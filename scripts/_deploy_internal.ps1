param($RunMigrationsValue = "false")
...
docker build -t $ImageUri ..\src\YandexCloudCourseTask.API
...
yc serverless container revision deploy `
  ...
  --environment "DB_ENDPOINT=$env:DB_ENDPOINT,DB_NAME=$env:DB_NAME,BACKEND_VERSION=$ImageTag,ASPNETCORE_URLS=http://+:8080,RUN_MIGRATIONS=$RunMigrationsValue" `
...
if ($Gateway.attached_domains) {
    $AllDomains += $Gateway.attached_domains.domain_name
}
